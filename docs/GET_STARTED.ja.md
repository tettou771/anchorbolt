...
会場マシン上のアプリの設定ファイルに `sinks` 配列を追加します。プリセットがテンプレートを埋めてくれます：
...
ステップ5では、1台のマシンに sinks を設定しました。サーバー側でフリート全体の通知を設定することもできます：歯車アイコン →**Notify** タブを開いてください。ここで追加した sink はサーバーの `config/notify.json` に保存され、すべてのアプリに対して発火します——マシンごとに設定を同期して回る必要はありません。

プリセットは同じものが使えます——**slack**、**discord**、**ntfy**、**uptime-kuma**——それに加えて **generic** があり、これを選ぶと `{{app}}`、`{{event}}`、`{{msg}}`、`{{time}}` のプレースホルダを持つ JSON ボディテンプレートが現れるので、他のどんなサービスにも対応できます。各 sink には2つのフィルタがあります：
...
- **scope** — グループ名または `app:<id>` エントリ。空欄はすべてのアプリを意味します。`app:<id>` を1つだけ指定した sink は、そのアプリのマシン側に同じ sink を設定したのと同じように振る舞います。
