...
リモートコントロールに必要なのは、operator ロール（サーバー側）と、アプリが `mcp::registerControlTools()` でオプトインしていること——これがゲートのすべてで、会場側フラグは不要です。control トグルはアプリが入力ツールを出しているときだけ現れます。監視だけなら HTTP のルートだけで足ります。
