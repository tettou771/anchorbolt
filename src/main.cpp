...
static void printHelp(bool verbose) {
    if (!verbose) {
        cout <<
            ...
            "                     URL persists per-install, so later runs need neither --server\n"
            "                     nor --pair. Pass --no-server to force local-only.\n"
    ...
    cout <<
        ...
        "  --generate-config  print a commented config template to stdout and exit\n"
        "  --config <file>    JSON config, comments allowed (auto-loads ./anchorbolt.json).\n"
        "                     Keys: app, args, cwd, port, wsPort, denyUpdate,\n"
        ...
        "                     'sinks' still accepted). No 'server'/'token'/'id' keys —\n"
        "                     the fleet URL is per-install (--server once) and the token\n"
        "                     is a secret (tokenFile or the env), so neither belongs in a\n"
        ...
        "                     assigned by the server, not set here. The URL is remembered\n"
        "                     alongside the token, so later runs need neither --server nor\n"
        "                     --pair. A --server that disagrees with the saved one prompts\n"
        "                     for a re-pair (empty input keeps the old identity intact).\n"
        "  --no-server        force local-only supervision; ignore any saved fleet URL and\n"
        "                     do not push heartbeats/logs/thumbnails. Keeps the token file\n"
        "                     intact so the next plain run reconnects to the saved fleet.\n"
        ...
        "                     fires the offline notification AND turns the wall dot\n"
        "                     red, one threshold (default 60)\n"
        "\n"
