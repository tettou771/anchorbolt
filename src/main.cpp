...
static void printHelp() {
    cout <<
        ...
        "                     allowUpdate, update (command array), grace, watchdogTimeout,\n"
        "                     log{dir,keepDays}, thumb{interval,width,quality}, tokenFile.\n"
        "                     A 'token' key is refused — keep secrets in tokenFile or the\n"
        "                     env, not in a config that lands in git.\n"
        ...
        "                     fails its first run; override via the config 'update' array,\n"
        "                     e.g. prepend \"trusscli upgrade\" to update TrussC itself)\n"
